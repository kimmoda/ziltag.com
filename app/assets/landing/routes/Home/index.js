...
class Home extends React.Component {
  ...
  componentDidMount() {
    if (window.innerWidth/window.innerHeight > this._cover.naturalWidth/this._cover.naturalHeight) {
      this._cover.style.width = window.innerWidth + 'px'
      this._cover.style.top = -Math.abs(window.innerHeight - this._cover.height)/2 + 'px'
    } else {
      this._cover.style.height = window.innerHeight + 'px'
      this._cover.style.left = -Math.abs(window.innerWidth - this._cover.width)/2 + 'px'
    }
  }
